  ...
  return (
    <main className="recorder-shell">
      ...
      <section className="submission-grid">
        ...
        <div className="recording-panel"><p className="eyebrow">02 · BÀI GHI ÂM CHÍNH THỨC</p><div className="recording-steps"><div className={`step ${part === 1 ? "active" : "done"}`}><span>{part === 2 ? "✓" : "1"}</span><div><strong>Luyện âm</strong><p>Đọc phần âm và từ theo yêu cầu trong bài.</p></div></div><div className={`step ${part === 2 ? "active" : ""}`}><span>2</span><div><strong>Trả lời Speaking Part 1</strong><p>Nói lần lượt các câu trả lời đã chuẩn bị.</p></div></div></div><div className="studio"><div className="studio-top"><span className={`status-dot ${recorderState}`} />{statusText}</div><div className="timer">{formatTime(elapsed)}</div><p>{part === 1 ? "Nội dung đang ghi: Luyện âm." : "Nội dung đang ghi: Trả lời Speaking Part 1."} Có thể dừng tạm sau từng câu rồi ghi tiếp.</p><div className="controls">{!micTesting ? <button className="secondary" onClick={startMicTest} disabled={recorderState !== "idle"}>{micTested ? "THỬ LẠI MICRO" : "THỬ MICRO"}</button> : <button className="secondary testing" onClick={stopMicTest}>DỪNG THỬ MICRO</button>}<button className="secondary" onClick={pauseRecording} disabled={recorderState !== "recording"}>DỪNG TẠM</button><button className="secondary" onClick={resumeRecording} disabled={recorderState !== "paused"}>GHI TIẾP</button></div>{micSampleUrl && <audio className="sample-player" src={micSampleUrl} controls><track kind="captions" /></audio>}<label className="confirm-row"><input type="checkbox" checked={acknowledged} onChange={(event) => setAcknowledged(event.target.checked)} disabled={recorderState !== "idle"} /><span>Em đã luyện kỹ, đã thử micro và hiểu rằng bài ghi chính thức chỉ có 01 lượt.</span></label>{recorderState === "idle" && <button className="primary" onClick={startOfficialRecording} disabled={!readyToStart}>BẮT ĐẦU BÀI GHI CHÍNH THỨC</button>}{part === 1 && (recorderState === "recording" || recorderState === "paused") && <button className="primary" onClick={nextPart}>HOÀN TẤT PHẦN 1 · SANG PHẦN 2</button>}{part === 2 && (recorderState === "recording" || recorderState === "paused") && <button className="primary finish" onClick={finishRecording}>HOÀN TẤT BÀI GHI ÂM</button>}{audioUrl && <div className="final-audio"><strong>Nghe lại bài ghi hoàn chỉnh</strong><audio src={audioUrl} controls><track kind="captions" /></audio><small>Chỉ để kiểm tra trước khi nộp; không có nút ghi lại.</small></div>}{error && <p className="inline-error">{error}</p>}</div></div>
      </section>
